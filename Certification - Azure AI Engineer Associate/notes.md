...
- Reward aggregation in Azure Personalizer allows for a nuanced evaluation of recommendations by combining multiple feedback signals into a comprehensive assessment. ex. Set to "earliest" to ensure only 1 count.
...
- Exported models Will be downloaded in a .zip archive and contain labels.txt and model.pb files. The labels.txt file contains classification labels used by your image classification model. You also should use model.pb. model.pb is a protocol buffer (ProtoBuf) file that contains your exported TensorFlow model. You can import it as a graph in your Python script.
...
- **Spatial Analysis** :  It can ingest a live video stream, detect and track people, monitor specific regions of interest on the video, and generate an event when a specific trigger occurs. Video summary, frame locator, count people in an area, detect position of people.
    -  set up an environment that supports Azure Media Services along with the Computer Vision Spatial Analysis container. You'll process the video feed through this service, and it will analyze the content in real-time or from recorded footage to identify and report on the presence of people.
...
speechConfig.AddTargetLanguage("de"); // Example: German as target language
speechConfig.SpeechRecognitionLanguage = "en-US";
speechConfig.SetProperty(PropertyId.SpeechServiceConnection_SynthVoice, "de-DE-Hedda");
using (var recognizer = new TranslationRecognizer(speechConfig))
{
    recognizer.Recognizing += (s, e) => {
        Console.WriteLine($"Recognizing: Text={e.Result.Text}, Translation={e.Result.Translations["de"]}");
    };
    recognizer.Recognized += (s, e) => {
        if (e.Result.Reason == ResultReason.TranslatedSpeech)
        {
            Console.WriteLine($"Recognized: Text={e.Result.Text}, Translation={e.Result.Translations["de"]}");
        }
    };
...
- An attribute or field being "facetable" means that it can be used to create facets, which are categories or buckets that group information based on the field's values. Facets are useful in search scenarios to provide aggregated data that helps users filter and navigate search results more efficiently. Ex : define the field attributes for a field where the search results will include a hit count by category.
